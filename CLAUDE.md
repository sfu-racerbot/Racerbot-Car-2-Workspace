...
- In the pinned F1TENTH Gym revision, gym's own collision check **never fires** with this workspace's vehicle parameters (`side_distances` computes to all zeros, and `range_min` 0.05 exceeds the 0.005 `ttc_threshold`) — a car can drive the whole circuit through walls unflagged. Neither simulator trusts it any more: `tools/f1tenth_sim/sim_fidelity/` checks the chassis against the map distance transform and does car-to-car overlap itself (`tools/f1tenth_sim/README.md`), and `racerbot_sim` samples the body against the occupancy grid. Do not reinstate a "no collision" claim that rests on gym's own flag.
