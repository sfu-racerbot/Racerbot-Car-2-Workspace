...
- **No → this is support/tooling code.** It only subscribes, or publishes things that can't move the car (visualizations, logs, diagnostics, recorded data). No deadman check, no interface contract — just needs to be a well-formed package (below) and shouldn't do anything surprising to topics it doesn't own. `web_dashboard` (a live browser dashboard — see [web-dashboard.md](web-dashboard.md)) is the reference example: it publishes to no topic, so none of the driving-code precautions apply to *running* it. Note that it does have one narrow write path — its [live tuning panel](web-dashboard.md#live-parameter-tuning) calls the driving nodes' `set_parameters` service. That still isn't driving code (it cannot command motion, only adjust a node that is already driving, within bounds that node enforces on itself), but it is the line worth noticing: publishing something that *can* move the car puts you in the other category, no matter how the code is packaged.
