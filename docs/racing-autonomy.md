...
1. Start with the defaults (`a_lat_max=2.5`, `a_accel_max=3.0`, `a_brake_max=3.0`, `v_max=4.0` — all SI units, m/s and m/s²). `a_brake_max` was 8.0 until that planned braking the car could not do and put it into the wall in its first simulated race — see `profile_max_brake` in `auto_map_race.yaml`. These are starting points for physical testing, not measured tire limits.
...
| `wheelbase` | `0.36` | Meters, measured on this car 2026-08-24; must match `vesc.yaml` |
...
| `off_racing_line_recovery_timeout_sec` | `5.0` | s; give up and hard-stop if recovery has not closed the error back under `max_cross_track_error` within this long. The give-up **latches** (2026-09-26): it holds until the error closes (car repositioned, or localization corrects) or LB is released, rather than re-arming a fresh crawl on the next tick |
...
- **`gap_follow` has no concept of "opponent" during the mapping phase at all.** It is deliberately map-free and reactive (see `gap_follow`'s own README) — every LIDAR return, wall or car, is just geometry to route around. Confirmed on `racerbot_sim`'s `traffic` scenario (2026-08-25, seed 12345): the ego can end up wedged against a moving opponent car during mapping, hard-stopping correctly (never faking an escape from a genuine flank pin — see the directional-escape gate above) but with no mechanism to ever recover once pinned, timing out the run. Verified this is not a regression from anything in this document's other 2026-08-25 entries: the same scenario and seed times out identically (worse, in fact — more sustained car-contact) on the commit immediately before them. A real fix needs `gap_follow` to either gain some opponent-awareness during mapping or hand mapping-phase traffic avoidance to a supervisor layer above it; out of scope here.
