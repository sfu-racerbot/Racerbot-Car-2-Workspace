...
- **Web dashboard's camera panel says "camera offline" even though the camera/stream nodes are definitely running**: you're almost certainly viewing the dashboard through an editor's forwarded port (VS Code, SSH `-L`) at `localhost:8080` instead of the car's real address — the dashboard itself works fine over a tunnel, but the camera panel specifically needs a second, unforwarded connection straight to port `9090`. See [web-dashboard.md#finding-the-cars-address-and-viewing-through-a-forwarded-port](web-dashboard.md#finding-the-cars-address-and-viewing-through-a-forwarded-port) for the fix.
