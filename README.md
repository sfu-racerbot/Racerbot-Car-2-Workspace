...
| `web_dashboard` | local package | live browser dashboard of the map/LIDAR/pose over a WebSocket, plus panels for tuning the driving nodes' parameters live, measuring distances on the map, and clearing or deleting a saved map — not an autonomy node, publishes to no topic, safe to run alongside anything else. Workflow: [docs/web-dashboard.md](docs/web-dashboard.md); code detail: [src/web_dashboard/README.md](src/web_dashboard/README.md) |
